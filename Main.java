...
public class Main {
    ...
    // Observador 1: Painel LCD
        sensor.addObserver(temp -> 
            System.out.println("[PAINEL LCD] Exibindo nova temperatura: " + temp + "°C")
        );

    // Observador 2: Ar Condicionado
        sensor.addObserver(temp -> {
            if (temp > 25.0) {
                System.out.println("[AR-CONDICIONADO] Temperatura alta. Ligando modo refrigeração");
            } else {
                System.out.println("[AR-CONDICIONADO] Temperatura agradável. Modo econômico ativado.");
            }
        });

    // Simulando mudanças de temperatura
    // logger.log("Mudando temperatura para 28.5°C");
    // sensor.setTemp(28.5);
    logger.log("Mudando temperatura para 22.0°C");
    sensor.setTemp(22.0);
...
interface HomeObserver { void update(double value); }

class SensorTemperatura {
    private List<HomeObserver> observers = new ArrayList<>();
    private double temp;

    public void addObserver(HomeObserver termostato) { observers.add(termostato); }
    ...
    private void notifyAllObservers() {
        for (HomeObserver termostato : observers) {
            termostato.update(temp);
        }
    }
}
